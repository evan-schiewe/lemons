...
export function parseRaceCsv(csvText: string): ParsedRaceCsv {
  ...
  dataLines.forEach((line, index) => {
    ...
    const repairedTokens = repairTokens(
      tokenizeCsvLine(line),
      warnings,
      rowNumber,
    );
    ...
    if (repairedTokens.length !== DEFAULT_HEADERS.length) {
      warnings.push(
        `Skipped row ${rowNumber}: expected 8 columns after repair, found ${repairedTokens.length}.`,
      );
      return;
    }
...
function repairTokens(
  ...
  if (tokens.length < DEFAULT_HEADERS.length) {
    ...
    warnings.push(
      `Padded short row ${rowNumber} from ${tokens.length} to 8 columns.`,
    );
    return padded;
  }
  ...
  warnings.push(
    `Repaired malformed row ${rowNumber} containing ${tokens.length} columns.`,
  );
...
function buildFieldMap(headers: string[]): Record<CsvHeader, number> {
  ...
  DEFAULT_HEADERS.forEach((header) => {
    const index = headers.findIndex((candidate) =>
      HEADER_ALIASES[header].includes(candidate),
    );
    if (index >= 0) {
